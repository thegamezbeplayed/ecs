...
param_t ElementOwnerChildren(void* p){
  ...
  for(int i = 0; i < e->owner->num_children; i++){
    ...
    others[count++] = ParamMakeObj(DATA_ELEM, c->uid, c);
  }
...
param_t ElementNiblings(void *p){
  ...
  e->ctx = ParamMakeArray(DATA_ELEM, e->uid, sib->children, sib->num_children, sizeof(ui_element_t*));
  ...
  for(int i = cur_child; i < sib->num_children; i++){
    ...
    child->ctx = ParamMakeObj(DATA_ELEM, sib->children[i]->uid, sib->children[i]);
  }
  ElementSetState(e, ELEMENT_IDLE);
}
